...
# Matches governance/config files that are always writable regardless of active step.
# Uses re.search so it works on both relative (".agents/...") and absolute paths.
ALWAYS_ALLOWED = re.compile(
    r"(CURRENT_TASK\.md|ROADMAP\.md|CHANGELOG\.md|KNOWN_ISSUES\.md|LEARNINGS\.md"
    r"|README\.md|ARCHITECTURE\.md|TESTING\.md|\.gitignore|\.env\.example"
    r"|\.agents[/\\]|[/\\]notes[/\\]|notes[/\\]|\.gitkeep)"
)
...
def main():
    ...
    # Step check is the single source of truth — must be in Execution to write source files.
